...
export function InvoiceForm({ invoiceData, setInvoiceData }: InvoiceProps) {
  ...
  const updateInvoiceItem = (index: number, field: string, value: string | number) => {
    setInvoiceData((prev) => {
      ...
      const updatedQuantity = field === 'quantity' ? Number(value) : Number(updatedItem.quantity);
      const updatedPrice = field === 'price' ? Number(value) : Number(updatedItem.price);
      ...
      const total = includeVAT ? subtotal * (1 + prev.totals.vat / 100) : subtotal;
  ...
  const handleAddProductToDatabase = async (product: { description: string; price: number }) => {
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">Toggle 15% VAT calculation</p>
                  </div>
                  <Switch
                    checked={includeVAT}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span>15% VAT</span>
                    <span>{includeVAT ? invoiceData.totals.vat.toFixed(2) : '0.00'} %</span>
                  </div>
